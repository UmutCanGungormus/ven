...
class Slide_model extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->column_order = array('rank', 'id', 'id', 'title', 'description', 'img_url','isActive');
        // Set searchable column fields
        $this->column_search = array('rank', 'id', 'id', 'title', 'description','img_url', 'isActive');
        // Set default order
    ...
    private function _get_datatables_query($postData)
    {
        ...
        if (!empty($this->input->post('search'))) {
            ...
            $this->db->group_start();
            $this->db->like('description', $this->input->post('search'), 'both');
            $this->db->group_end();
        }
        ...
        foreach ($this->column_search as $item) {
            ...
            if (!empty($postData['search'])  && ($item == "title" || $item == "description")) {
                // first loop
                if ($i === 0) {
                    // open bracket
                    $this->db->group_start();

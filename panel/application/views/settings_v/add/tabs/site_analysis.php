<div role="tabpanel" class="tab-pane fade" id="tab-9">
	<div class="row">
		<div class="form-group col-md-12">
			<label>Google Analytics (Script kodlarını editörün "Code View (< / >)" bölümünden ekleyiniz!)</label>
			<textarea name="analytics" class="m-0 tinymce" data-plugin="summernote" data-options="{height: 250}"></textarea>
		</div>
		<div class="form-group col-md-12">
			<label>Yandex Metrica (Script kodlarını editörün "Code View (< / >)" bölümünden ekleyiniz!)</label>
			<textarea name="metrica" class="m-0 tinymce" data-plugin="summernote" data-options="{height: 250}"></textarea>
		</div>
	</div>
</div>
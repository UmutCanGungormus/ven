...
class Services extends MY_Controller
{
    ...
    public function datatable()
    {
        ...
        foreach ($items as $item) {
            ...
            $checkbox = '<div class="custom-control custom-switch"><input data-id="' . $item->id . '" data-url="' . base_url("services/check") . '" data-status="' . ($item->isActive == 1 ? "checked" : null) . '" id="customSwitch' . $i . '" type="checkbox" ' . ($item->isActive == 1 ? "checked" : null) . ' class="my-check custom-control-input" >  <label class="custom-control-label" for="customSwitch' . $i . '"></label></div>';
...
public function check()
{
    ...
    $product = $this->service_model->get(["id" => $id])->isActive;
    if ($product == 1) {
        $status = 0;
    } else {
        $status = 1;
    }
    if ($this->service_model->update(["id" => $id], ["isActive" => $status])) {

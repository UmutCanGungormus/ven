...
class News_box extends MY_Controller
{
        ...
        foreach ($items as $item) {
            ...
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
                    ...
                    <a class="dropdown-item" href="' . base_url("news_box/delete/$item->id") . '"><i class="fa fa-trash mr-2"></i>Kaydı Sil</a>
                    </div>
            </div>';
            ...
            $checkbox= '<div class="custom-control custom-switch"><input data-id="'.$item->id.'" data-url="'.base_url("news_box/check").'" data-status="'.($item->isActive == 1 ? "checked" : null).'" id="customSwitch'.$i.'" type="checkbox" '.($item->isActive == 1 ? "checked" : null).' class="my-check custom-control-input" >  <label class="custom-control-label" for="customSwitch'.$i.'"></label></div>';
    ...
    public function check()
    {
        ...
        $product = $this->news_box_model->get(["id" => $id])->isActive;
        if ($product == 1) {
            $status = 0;
        } else {
            $status = 1;
        }
        if ($this->news_box_model->update(["id" => $id], ["isActive" => $status])) {
